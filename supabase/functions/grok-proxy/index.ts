/**
 * Grok (xAI) Proxy Edge Function
 *
 * Proxies requests to xAI Grok API with:
...
import type { GrokProxyInput } from '../_shared/validation.ts';
...
const handler = createValidatedHandler<GrokProxyInput>(
  {
    schema: grokProxySchema,
    rateLimit: RATE_LIMIT_PRESETS.AI_API,
    functionName: 'grok-proxy',
  },
  async ({ user, data }) => {
    ...
    if (!isEndpointAllowed(endpoint)) {
      console.error(`[Grok Proxy] Blocked endpoint: ${endpoint}`);
      return new Response(
        JSON.stringify({
          error: 'FORBIDDEN',
    ...
    // Call Grok API
    // -------------------------------------------------------------------------
    const apiUrl = `https://api.x.ai${endpoint}`;
    console.log(`[Grok Proxy] ${user.id} -> ${method} ${endpoint}`);
    ...
    console.log(`[Grok Proxy] Response: ${apiResponse.status} (${duration}ms)`);
