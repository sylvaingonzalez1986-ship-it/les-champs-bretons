...
const updateSchema = insertSchema.partial().refine((data) => Object.keys(data).length > 0, {
  message: 'No updates provided',
});

const requestSchema = z.discriminatedUnion('action', [
  z.object({ action: z.literal('create'), track: insertSchema }),
  z.object({ action: z.literal('update'), id: z.string().min(1), updates: updateSchema }),
...
serve(async (req) => {
  ...
  return jsonResponse({ error: 'INVALID_ACTION' }, 400, corsHeaders);
});
