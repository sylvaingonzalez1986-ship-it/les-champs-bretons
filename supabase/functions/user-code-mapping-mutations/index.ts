...
serve(async (req) => {
  ...
  const { data: existingByCode, error: existingByCodeError } = await serviceClient
    .from('user_code_mapping')
    .select('user_id')
  ...
  if (existingByCode && existingByCode.user_id && existingByCode.user_id !== user.id) {
    return jsonResponse({ error: 'CONFLICT' }, 409, responseCorsHeaders);
  }

  const { data: existingByUser, error: existingByUserError } = await serviceClient
    .from('user_code_mapping')
    .select('user_code')
    .eq('user_id', user.id)
    .maybeSingle();
  ...
  if (existingByUser && existingByUser.user_code && existingByUser.user_code !== userCode) {
    return jsonResponse({ error: 'CONFLICT' }, 409, responseCorsHeaders);
  }
