...
async function isProducerOwner(
...
serve(async (req) => {
  ...
  const rateLimitResult = await checkRateLimit(user.id, RATE_LIMIT_PRESETS.UPLOADS);
  if (!rateLimitResult.allowed) {
    return createRateLimitResponse(rateLimitResult, RATE_LIMIT_PRESETS.UPLOADS, corsHeaders);
  }
  ...
  if (role === 'producer') {
    const ownsProducer = await isProducerOwner(serviceClient, producerId, user.id);
    if (!ownsProducer) {
      return jsonResponse({ error: 'FORBIDDEN' }, 403, corsHeaders);
    }
  }

  const bucket = 'lab-analyses';
