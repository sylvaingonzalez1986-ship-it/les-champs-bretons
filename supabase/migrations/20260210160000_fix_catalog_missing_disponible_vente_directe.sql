-- Fix: producers_catalog materialized view was missing disponible_vente_directe
-- in the product JSONB, causing the local market to show zero producers.

DROP MATERIALIZED VIEW IF EXISTS producers_catalog;

CREATE MATERIALIZED VIEW producers_catalog AS
SELECT
  p.id,
  p.name,
  p.email,
  ...
  COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'disponible_vente_directe', prod.disponible_vente_directe,
