-- Add shipping options for direct sales (profiles, producers, local_market_orders)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'local_market_orders_delivery_method_check'
  ) THEN
    ALTER TABLE local_market_orders
      ADD CONSTRAINT local_market_orders_delivery_method_check
      CHECK (delivery_method IN ('pickup', 'shipping'));
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'local_market_orders_delivery_fee_check'
  ) THEN
    ALTER TABLE local_market_orders
      ADD CONSTRAINT local_market_orders_delivery_fee_check
      CHECK (delivery_fee >= 0);
  END IF;
END $$;

-- Rebuild materialized view to include shipping fields
...
GROUP BY
  ...
  p.shipping_enabled, p.shipping_fee, p.shipping_note,
