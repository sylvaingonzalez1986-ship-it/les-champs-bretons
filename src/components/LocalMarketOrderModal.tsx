...
interface ProductInfo {
  ...
  image?: string;
  stock?: number;
  price_tiers?: PriceTier[]; // Paliers de prix dégressifs
}
              ...
              {step === 'details' && (
                <>
                  ...
                  <View className="mb-6">
                    ...
                    {/* Indication du prochain palier */}
                    {nextTier && (
                      <View
                        className="flex-row items-center mt-3 p-2 rounded-lg"
                        style={{ backgroundColor: `${COLORS.accent.hemp}15` }}
                      >
                        <TrendingDown size={14} color={COLORS.accent.hemp} />
                        <Text className="ml-2 text-xs" style={{ color: COLORS.accent.hemp }}>
                          Encore {nextTier.minQuantity - quantity} pour {nextTier.price.toFixed(2)}€/unité !
                        </Text>
                      </View>
                    )}
                  </View>
                  ...
                  {product.price_tiers && product.price_tiers.length > 0 && (
                    <View className="mb-6">
                      <View className="flex-row items-center mb-3">
                        <Layers size={16} color={COLORS.primary.gold} />
                        <Text className="ml-2 text-sm font-semibold" style={{ color: COLORS.text.lightGray }}>
                          Tarifs dégressifs
                        </Text>
                      </View>
                      <View
                        className="rounded-xl p-3"
                        style={{ backgroundColor: `${COLORS.primary.gold}10`, borderWidth: 1, borderColor: `${COLORS.primary.gold}25` }}
                      >
                        {/* Prix de base */}
                        <View className="flex-row justify-between items-center mb-2">
                          <Text className="text-xs" style={{ color: COLORS.text.muted }}>
                            1 - {product.price_tiers[0] ? product.price_tiers[0].minQuantity - 1 : '∞'} unités
                          </Text>
                          <Text
                            className="font-semibold"
                            style={{ color: quantity < (product.price_tiers[0]?.minQuantity || 1) ? COLORS.primary.gold : COLORS.text.muted }}
                          >
                            {product.price_public.toFixed(2)}€
                          </Text>
                        </View>
                        ...
                        {[...product.price_tiers]
                          ...
                          .map((tier, index, arr) => {
                            const nextMin = arr[index + 1]?.minQuantity;
                            const isActive = quantity >= tier.minQuantity && (!nextMin || quantity < nextMin);
                            return (
                              <View key={index} className="flex-row justify-between items-center mb-1">
                                ...
                                <Text
                                  className="font-semibold"
                                  style={{ color: isActive ? COLORS.primary.gold : COLORS.text.muted }}
                                >
                                  {tier.price.toFixed(2)}€
                                  {isActive && (
                                    <Text style={{ color: COLORS.accent.hemp }}> ✓</Text>
                                  )}
                                </Text>
                              </View>
                            );
                ...
                <>
                  ...
                  >
                    ...
                    <View className="flex-row justify-between mb-2">
                      ...
                      <View className="flex-row items-center">
                        {unitPrice !== product.price_public && (
                          <Text className="mr-2 line-through text-xs" style={{ color: COLORS.text.muted }}>
                            {product.price_public.toFixed(2)}€
                          </Text>
                        )}
                        <Text style={{ color: unitPrice !== product.price_public ? COLORS.accent.hemp : COLORS.text.cream }}>
                          {unitPrice.toFixed(2)}€
                        </Text>
                      </View>
                    </View>
