...
export default function ChatProducteursScreen() {
  ...
  const { isProducer, isAdmin, isPro } = usePermissions();
  const { email: authEmail, fullName } = useUserIdentity();

  // Accès au chat: producteurs, pros ET admins
  const hasAccess = isProducer || isAdmin || isPro;
          ...
          <View style={{ paddingTop: insets.top }} className="flex-1 items-center justify-center px-6">
            <Animated.View entering={FadeIn.duration(500)} className="items-center">
              ...
              <Text className="text-gray-400 text-center text-base mb-6">
                Le chat communautaire est réservé aux producteurs et professionnels partenaires des Chanvriers Unis.
              </Text>
              <Pressable
                onPress={() => router.back()}
          ...
          <View className="flex-row items-center justify-between px-4 py-3">
            <View className="flex-row items-center flex-1">
              ...
              <View className="flex-1">
                <Text className="text-white font-bold text-lg">Agora</Text>
