...
const ProductCard = ({
  ...
  // Tarifs dégressifs - afficher uniquement ceux correspondant au rôle de l'utilisateur
  const userPriceTiers = isPro ? product.priceProTiers : product.priceTiers;
  const hasUserPriceTiers = userPriceTiers && userPriceTiers.length > 0;
    ...
    <>
      ...
      >
        ...
        <View className="flex-1 py-3 pr-3">
          ...
          {/* Tarifs dégressifs indicator - uniquement pour le rôle de l'utilisateur */}
          {hasUserPriceTiers && (
            <Pressable
              onPress={() => setShowPriceTiers(!showPriceTiers)}
              className="mt-2"
            >
              <View
                className="flex-row items-center px-2 py-1 rounded-lg"
                style={{ backgroundColor: isPro ? `${COLORS.accent.teal}15` : `${COLORS.primary.brightYellow}15` }}
              >
                <Layers size={12} color={isPro ? COLORS.accent.teal : COLORS.primary.brightYellow} />
                <Text style={{ color: isPro ? COLORS.accent.teal : COLORS.primary.brightYellow }} className="text-xs font-medium ml-1">
                  Tarifs dégressifs {showPriceTiers ? '▲' : '▼'}
                </Text>
              </View>
              {showPriceTiers && userPriceTiers && (
                <View className="mt-2">
                  <View
                    ...
                    style={{
                      ...
                      borderWidth: 1,
                      borderColor: isPro ? `${COLORS.accent.teal}30` : `${COLORS.primary.brightYellow}30`
                    }}
                  >
                    <View className="flex-row items-center mb-1">
                      {isPro && <Briefcase size={10} color={COLORS.accent.teal} />}
                      <Text style={{ color: isPro ? COLORS.accent.teal : COLORS.primary.brightYellow }} className={`text-xs font-bold ${isPro ? 'ml-1' : ''}`}>
                        {isPro ? 'Tarifs Pros' : 'Tarifs Clients'}
                      </Text>
                    </View>
                    {[...userPriceTiers]
                      ...
                      .map((tier, idx) => (
                        <View key={idx} className="flex-row items-center justify-between py-0.5">
                          ...
                          <Text style={{ color: isPro ? COLORS.accent.teal : COLORS.primary.brightYellow }} className="text-xs font-bold">
                            {tier.price.toFixed(2)}€/{product.weight}
                          </Text>
                        </View>
                      ))}
                  </View>
...
export default function ShopScreen() {
  ...
  // Handle add to cart from product detail modal
  const handleAddToCartFromModal = (quantity: number) => {
    if (!selectedProduct || !producer) return;
    for (let i = 0; i < quantity; i++) {
      if (selectedProduct.isOnPromo && selectedProduct.promoPercent) {
        addToCart(selectedProduct, producer.id, producer.name, selectedProduct.promoPercent);
      } else {
        addToCart(selectedProduct, producer.id, producer.name);
      }
    }
  };
