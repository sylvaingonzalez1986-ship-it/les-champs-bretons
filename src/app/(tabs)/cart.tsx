...
import { ShoppingCart, Trash2, Plus, Minus, ArrowLeft, Package, Sparkles, MapPin, CheckCircle, AlertCircle, X, User, Ticket, Gift, Truck, Building2, Briefcase, Layers } from 'lucide-react-native';
...
export default function CartScreen() {
  ...
  // Helper function to get base price with tiers (Pro or Public)
  const getBasePrice = (item: typeof items[0]) => {
    // Utiliser la fonction de tarification par paliers
    return getPriceForQuantity(item.product, item.quantity, isPro);
  };
  ...
  // Helper to check if item has tiered pricing and current tier is different from base
  const hasTieredDiscount = (item: typeof items[0]) => {
    const tieredPrice = getPriceForQuantity(item.product, item.quantity, isPro);
    const basePrice = getBasePriceWithoutTiers(item);
    return tieredPrice < basePrice;
  };

  // Helper to get tier discount percentage
  const getTierDiscountPercent = (item: typeof items[0]) => {
    const tieredPrice = getPriceForQuantity(item.product, item.quantity, isPro);
    const basePrice = getBasePriceWithoutTiers(item);
    if (basePrice > 0 && tieredPrice < basePrice) {
      return Math.round((1 - tieredPrice / basePrice) * 100);
    }
    return 0;
  };
                        ...
                        {item.promoDiscount ? (
                          ...
                          <View className="mt-1">
                            <View className="flex-row items-center">
                              ...
                              >
                                <Layers size={10} color={COLORS.primary.brightYellow} />
                                <Text style={{ color: COLORS.primary.brightYellow, fontSize: 10, fontWeight: '700', marginLeft: 3 }}>
                                  PALIER
                                </Text>
                              </View>
                              <Text style={{ color: COLORS.text.muted, textDecorationLine: 'line-through' }} className="text-sm">
                                {`${getBasePriceWithoutTiers(item).toFixed(2)}€`}
                              </Text>
                            </View>
                            <View className="flex-row items-center">
                              ...
                              <View className="bg-yellow-500/20 px-1.5 py-0.5 rounded ml-2">
                                <Text style={{ color: COLORS.primary.brightYellow }} className="text-xs font-bold">
                                  {`-${getTierDiscountPercent(item)}%`}
                                </Text>
                              </View>
                            </View>
                          </View>
