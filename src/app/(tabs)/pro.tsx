...
import { ProProductDetailModal } from '@/components/ProProductDetailModal';
...
export default function ProScreen() {
  ...
  // État pour le modal de détail produit
  const [selectedProduct, setSelectedProduct] = useState<{
    product: ProducerProduct;
    producer: Producer;
  } | null>(null);

  // Combinaison des producteurs: admin = local, non-admin = Supabase synced
  ...
  // Ajouter au panier
  const handleAddToCart = (product: ProducerProduct, producer: Producer, quantity: number = 1) => {
    ...
    setAddedProductName(product.name + (quantity > 1 ? ` (x${quantity})` : ''));
    setShowAddedToast(true);
    setTimeout(() => setShowAddedToast(false), 2000);
  };

  // Ouvrir le modal de détail produit
  const openProductDetail = (product: ProducerProduct, producer: Producer) => {
    setSelectedProduct({ product, producer });
  };
  ...
  // Ajouter depuis le modal avec quantité
  const handleAddFromModal = (quantity: number) => {
    if (selectedProduct) {
      handleAddToCart(selectedProduct.product, selectedProduct.producer, quantity);
    }
  };
  ...
  const renderProductCard = (
    ...
    return (
      ...
      >
        <Pressable
          onPress={() => openProductDetail(product, producer)}
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: COLORS.background.dark }}>
      ...
      >
        <View className="flex-row items-center mb-4">
          ...
          {(isPro || isAdmin || isProApproved) && (
            ...
            >
              <ShoppingCart size={16} color={COLORS.primary.gold} />
              <Text style={{ color: COLORS.primary.gold }} className="text-xs font-medium ml-1">
                Commandes
              </Text>
            </Pressable>
          )}
      ...
      <ProProductDetailModal
        visible={selectedProduct !== null}
        onClose={closeProductDetail}
        product={selectedProduct?.product ?? null}
        producer={selectedProduct?.producer ?? null}
