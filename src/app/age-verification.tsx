...
// Fonction dédiée pour la confirmation d'âge (crée le profil si nécessaire)
async function confirmAgeDirectly(retryCount = 0): Promise<{ success: boolean; error: string | null }> {
  ...
  try {
    ...
    // D'abord, essayer PATCH pour mettre à jour un profil existant
    ...
    if (!patchResponse.ok) {
      return { success: false, error: `Erreur ${patchResponse.status}: ${patchText}` };
    }
