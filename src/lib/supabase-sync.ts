...
export async function fetchOrders(producerId?: string): Promise<Order[]> {
  ...
  try {
    ...
    console.log('[fetchOrders] Fetching orders from:', url);
    ...
    if (!response.ok) {
      ...
      console.log('[fetchOrders] Error response:', response.status, errorText);
      ...
      if (response.status === 403) {
        ...
        console.log('[fetchOrders] This usually means RLS policies need to be updated for producer access');
        return [];
      }
    ...
    console.log('[fetchOrders] Successfully fetched', data.length, 'orders');
...
export async function sendChatMessage(
  ...
  console.log('[Chat] sendChatMessage called:', { senderId, senderName, contentLength: content.length });
  ...
  try {
    ...
    console.log('[Chat] Sending to:', url);
    ...
    if (!response.ok) {
      const error = await response.text();
      console.warn('[Chat] Erreur envoi message:', response.status, error);
      return null;
    }
...
export async function fetchChatMessages(limit: number = 50): Promise<ChatMessage[]> {
  console.log('[Chat] fetchChatMessages called, limit:', limit);
  ...
  try {
    // Utiliser les headers authentifiés pour que RLS puisse vérifier auth.uid()
    const headers = await getAuthenticatedHeaders();
    const url = `${SUPABASE_URL}/rest/v1/chat_messages?order=created_at.desc&limit=${limit}`;
    ...
    if (!response.ok) {
      const error = await response.text();
      console.warn('[Chat] Erreur fetch messages:', response.status, error);
      return [];
    }

    const data: SupabaseChatMessage[] = await response.json();
    console.log('[Chat] Messages fetched from Supabase:', data.length);
...
function connectWebSocket(): void {
  ...
  try {
    ...
    realtimeSocket.onopen = () => {
      ...
      // Vérifier que le socket est bien ouvert avant d'envoyer
      if (realtimeSocket?.readyState === WebSocket.OPEN) {
        realtimeSocket.send(JSON.stringify(joinMessage));
      } else {
        console.warn('[Chat] WebSocket not ready, state:', realtimeSocket?.readyState);
      }
